...
export default function OperatorBusDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        pageTitle={bus.plateNumber}
        pageDescription="Bus details – read-only view"
      />

      <div className="p-6 space-y-6">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-1.5 text-sm text-gray-500">
          <Link href="/operator/dashboard" className="hover:text-blue-600 transition-colors">
            Dashboard
          </Link>
          <ChevronRight className="w-4 h-4 shrink-0" />
          <Link href="/operator/fleet-management" className="hover:text-blue-600 transition-colors">
            Fleet Management
          </Link>
          <ChevronRight className="w-4 h-4 shrink-0" />
          <span className="font-medium text-gray-800 truncate">{bus.plateNumber}</span>
        </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-gray-900 truncate">{bus.plateNumber}</h1>
            <p className="text-sm text-gray-500">
              {bus.manufacturer} {bus.model} · {bus.year} ·{' '}
              <span className="font-mono">{bus.ntcRegistrationNumber}</span>
            </p>
          </div>
